...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const verifySubscription = async (pageId, accessToken) => {
        try {
          ...
          return response.data.data?.some(
            (app) => app.id === process.env.FB_APP_ID
          );
        } catch (error) {
      ...
      res.redirect(
        `http://localhost:5173?token=${token}&user_id=${facebookAccount.id}`
      );
    } catch (error) {
      console.error("Error:", error);
      res.redirect(
        `http://localhost:5173/error?message=${encodeURIComponent(
          error.message
        )}`
      );
    }
